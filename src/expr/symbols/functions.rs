...
fn eval_shared_value<'a>(
    ...
    match sv {
        ...
        _ => {
            return Err(format!("value {:?} cannot be used as a function", sv));
        }
    }
}

fn eval_owned_value<'a>(ov: &OwnedValue, params: Vec<SExpr<'a>>) -> Result<SExpr<'a>, String> {
    match ov {
        ...
        _ => {
            return Err(format!("value {:?} cannot be used as a function", ov));
        }
    }
}
