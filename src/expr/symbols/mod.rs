...
impl ISymbolMap {
    ...
    pub fn insert(&self, symbol_id: u64, symbol_impl: Box<Symbol>) -> Result<(), ()> {
        match self.map.try_borrow_mut() {
            Ok(ref mut m) => {
                m.insert(symbol_id, symbol_impl);
                Ok(())
            },
            Err(_) => Err(())
        }
...
pub fn new_symbol(symbol_id: u64, symbol_impl: Box<Symbol>) -> Result<(), ()> {
